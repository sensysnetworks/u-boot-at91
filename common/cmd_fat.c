...
U_BOOT_CMD(
	...
	"store binary file to a dos filesystem",
	"<interface> <dev[:part]>  <addr> <filename> [bytes]\n"
	"    - store binary file 'filename' to 'dev' on 'interface'\n"
	"      from address 'addr' to dos filesystem\n"
);
...
int do_fat_del(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int dev=0;
	int part=1;
	char *ep;
	...
	if (argc < 4) {
		printf ("usage: fatdel <interface> <dev[:part]> <filename>\n");
		return (0);
	}
	...
	if (fat_register_device(dev_desc,part)!=0) {
		printf ("\n** Unable to use %s %d:%d for fatdel **\n",argv[1],dev,part);
		return 1;
	}

	ret = file_fat_unlink (argv[3]);

	return (ret);
}

U_BOOT_CMD(
	fatdel,	4,	1,	do_fat_del,
	"delete a file",
	"<interface> <dev[:part]> [filename]\n"
	"    - delete 'filename' from 'dev' on 'interface'\n"
);
...
int do_fat_dump (cmd_tbl_t *cmdtp, bd_t *bd, int flag, int argc, char *argv[])
{
	__u8 block[1024];
	int ret;
	int bknum;
