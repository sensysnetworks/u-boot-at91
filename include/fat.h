...
#define CHECK_CLUST(x, fatsize) ((x) <= 1 || \
 (x) >= ((fatsize) == 32 ? 0xffffff0 : (fatsize) == 16 ? 0xffff : 0x0fff ))
...
typedef struct {
  ...
  int   rootdir_size;	      /* Number of sectors in root_dir */
  int   fats;		            /* Number of FATs */
  __u32 fatlast;	          /* Length of FAT in sectors */
  int	fat_dirty;
  int   fat_in_use;             
  int   fat_broken[MAXFATFATS];
  __u32 last_fat_sector;
  __u32 last_dir_sector;
  __u32 last_dir_size;
} fsdata;

typedef int	  (file_detectfs_func)(void);
typedef int	  (file_ls_func)(const char *dir);
typedef int	  (file_ls_none_func)(const char *dir);
typedef long	(file_read_func)(const char *filename, void *buffer,unsigned long maxsize);
typedef long	(file_write_func)(const char *filename, void *buffer,unsigned long maxsize);
typedef int	  (file_unlink_func)(const char *filename);
typedef int	  (file_rename_func)(const char *oldfilename,const char *newfilename);
...
struct filesystem {
	...
	file_unlink_func	  *unlink;
	file_rename_func	  *rename;
	file_ls_none_func   *ls_none;
	const char	    name[12];
};
...
file_write_func		  file_fat_write;
file_unlink_func		file_fat_unlink;
file_rename_func		file_fat_rename;
file_ls_none_func	  file_fat_ls_none;
