			    ...
			    off_t offset, uint8_t mtd_type)
{
	...
	{
		top_of_range = DEVOFFSET (dev) + ENVSECTORS (dev) * blocklen;
		erase_offset = offset;
	}
	else
	{
	...
	if ((mtd_type == MTD_NANDFLASH) || (mtd_type == MTD_DATAFLASH)) {
		/*
...
static int flash_read (int fd)
{
	...
	if (mtdinfo.type != MTD_NORFLASH && mtdinfo.type != MTD_NANDFLASH && mtdinfo.type != MTD_DATAFLASH) {
